...
const Profile: React.FC<ProfileProps> = ({ 
  ...
  const handleSaveProfile = () => {
    if (!editForm.name || !editForm.address || !editForm.selectedZone) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-10">
      <div className="flex flex-col items-center py-6">
        ...
        <div className="flex gap-2 mt-4">
          <button 
            ...
            className="text-[8px] font-black bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-300 border border-slate-100 dark:border-slate-700 px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1.5 active:scale-95 transition-all shadow-sm"
          >
            <i className={`fas ${isEditing ? 'fa-times' : 'fa-pen-to-square'}`}></i>
      ...
      <div className="space-y-4">
        {isEditing ? (
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-6 shadow-xl border border-brand-50 dark:border-slate-700 space-y-5 animate-in slide-in-from-bottom-4">
            <h3 className="text-[10px] font-black text-brand-700 uppercase tracking-widest ml-1 mb-2">Update Details</h3>
            
            <div className="space-y-4">
              <div className="text-left">
                <label className="text-[9px] uppercase font-black text-slate-400 mb-1.5 block ml-1">Full Name</label>
                <input 
                  type="text" 
                  value={editForm.name} 
                  onChange={(e) => setEditForm({...editForm, name: e.target.value})} 
                  placeholder="Enter name" 
                  className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white transition-all shadow-sm focus:border-brand-500" 
                />
              </div>

              <div className="text-left">
                <label className="text-[9px] uppercase font-black text-slate-400 mb-1.5 block ml-1">Home Address</label>
                <textarea 
                  value={editForm.address} 
                  onChange={(e) => setEditForm({...editForm, address: e.target.value})} 
                  placeholder="Building, Door No, Street..." 
                  className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white transition-all shadow-sm focus:border-brand-500 h-20 resize-none" 
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="text-left">
                  <label className="text-[9px] uppercase font-black text-slate-400 mb-1.5 block ml-1">Pincode</label>
                  <input 
                    type="tel" 
                    value={editForm.pincode} 
                    onChange={(e) => setEditForm({...editForm, pincode: e.target.value.replace(/\D/g, '').slice(0, 6)})} 
                    placeholder="6-digits" 
                    className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white transition-all shadow-sm focus:border-brand-500 text-center" 
                  />
                </div>
                <div className="text-left">
                  <label className="text-[9px] uppercase font-black text-slate-400 mb-1.5 block ml-1">Delivery Zone</label>
                  <select 
                    value={editForm.selectedZone} 
                    onChange={(e) => setEditForm({...editForm, selectedZone: e.target.value})} 
                    className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white transition-all shadow-sm focus:border-brand-500"
                  >
            ...
            <button 
              ...
              className="w-full bg-brand-700 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all mt-4"
            >
              Save Changes
            </button>
          </div>
